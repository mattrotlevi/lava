...
def start(args):
	...
	help_str = "usage:\n\texec cosmos_db_get_conn_strings ---> scans subscription for cosmos db connection strings"
	...
	account_conn_strings = []
	for i in range(len(accts)):
		ret = subprocess.run(['az', 'storage', 'account', 'show-connection-string', '--name', accts[i]], stdout=subprocess.PIPE)
		if(ret.returncode != 0):
			print("[-] error: could not list connection strings [-]")
			return
		json_obj = json.loads(ret.stdout.decode('utf-8'))
		account_conn_strings.append(json_obj[0]['value'])

	pprint.pprint(account_conn_strings)
